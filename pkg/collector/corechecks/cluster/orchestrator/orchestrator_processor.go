...
func processRoleBindingList(roleBindingList []*rbacv1.RoleBinding, groupID int32, cfg *config.OrchestratorConfig, clusterID string) ([]model.MessageBody, error) {
	...
	for _, roleBinding := range roleBindingList {
		if orchestrator.SkipKubernetesResource(roleBinding.UID, roleBinding.ResourceVersion, orchestrator.K8sRole) {
			continue
		}
	...
	log.Debugf("Collected & enriched %d out of %d role bindings in %s", len(roleBindingMsgs), len(roleBindingList), time.Since(start))
	return messages, nil
}
...
func processClusterRoleList(clusterRoleList []*rbacv1.ClusterRole, groupID int32, cfg *config.OrchestratorConfig, clusterID string) ([]model.MessageBody, error) {
	...
	for _, clusterRole := range clusterRoleList {
		if orchestrator.SkipKubernetesResource(clusterRole.UID, clusterRole.ResourceVersion, orchestrator.K8sRole) {
			continue
		}
	...
	log.Debugf("Collected & enriched %d out of %d cluster roles in %s", len(clusterRoleMsgs), len(clusterRoleList), time.Since(start))
	return messages, nil
}
...
func processClusterRoleBindingList(clusterRoleBindingList []*rbacv1.ClusterRoleBinding, groupID int32, cfg *config.OrchestratorConfig, clusterID string) ([]model.MessageBody, error) {
	...
	for _, clusterRoleBinding := range clusterRoleBindingList {
		if orchestrator.SkipKubernetesResource(clusterRoleBinding.UID, clusterRoleBinding.ResourceVersion, orchestrator.K8sRole) {
			continue
		}
	...
	log.Debugf("Collected & enriched %d out of %d cluster role bindings in %s", len(clusterRoleBindingMsgs), len(clusterRoleBindingList), time.Since(start))
	return messages, nil
}
...
func processServiceAccountList(serviceAccountList []*corev1.ServiceAccount, groupID int32, cfg *config.OrchestratorConfig, clusterID string) ([]model.MessageBody, error) {
	...
	for _, serviceAcount := range serviceAccountList {
		if orchestrator.SkipKubernetesResource(serviceAcount.UID, serviceAcount.ResourceVersion, orchestrator.K8sRole) {
			continue
		}
	...
	log.Debugf("Collected & enriched %d out of %d service accounts in %s", len(serviceAccountMsgs), len(serviceAccountList), time.Since(start))
	return messages, nil
}
